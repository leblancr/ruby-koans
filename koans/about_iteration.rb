...
class AboutIteration < Neo::Koan
  ...
  def test_iterating_with_each
    array = [1, 2, 3]
    sum = 0
    array.each do |item| # each method, do/end call each thing 'item'
      sum += item
    end
    assert_equal 6, sum
  end
  ...
  def test_each_can_use_curly_brace_blocks_too
    array = [1, 2, 3]
    sum = 0
    array.each { |item| sum += item } # brackets, call each thing 'item'
    assert_equal 6, sum
  end
  ...
  def test_inject_will_blow_your_mind
    ...
    # starts with an initial value of 1, can't multiply by 0
    result2 = [2, 3, 4].inject(1) { |product, item| product * item }
    assert_equal 24, result2

    # Extra Credit:
    # Describe in your own words what inject does.
    # inject (also known as reduce) method to accumulate a value over an array.
  end
  ...
  #   # code to read 'file'
  #
  # Block form automatically closes, second one must file.close()
  #
